...
7. Propose a plan with `propose_plan` before beginning execution (minor EDA is an exception). The plan must include a concise `name`, a clear `description`, and ordered `steps`. Each step should have `name`, `description`, and `status`; use `not_started` for steps not yet begun. Leave `summary` empty unless the step already has completed EDA findings, and include `outputs` only for files that already exist.
