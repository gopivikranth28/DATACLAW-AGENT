...
It combines OpenClaw, gBrain-style memory, notebook execution, and a custom analytics harness. It is an experimental beta and is built primarily with AI assistance. Release 3 adds a governed structured-EDA workflow, analytical review gates, versioned artifacts, a reader-first storyboard report builder with evidence-bound claim contracts and publish-time integrity gates, and a session-centric chat console.
...
| Report builder | Reader-first story arcs, typed sections, evidence-bound claim contracts, editorial review, runtime visual authoring, desktop browser outcome checks, a v8 quality rubric, regeneration recipes, and explicit draft → designed → published states | Build narrative reports with metrics, evidence-backed charts, interactive tables and linked explorers, methodology, and source-bound publish receipts |
...
4. In Chat, ask for a complete report or ask the agent to continue a draft. The agent selects the appropriate report workflow, plans a reader-facing story only from typed source facts, and surfaces whether the result is a draft, designed report, blocked publication, or published report.
5. For a release requiring visual approval, inspect the generated desktop browser evidence and record an approved visual review. The gate also checks deterministic rendered-page semantics such as hierarchy, evidence context, and editorial findings.
...
The designer defaults to a reader-first report rather than a fixed template: it composes a variable set of named story arcs, puts charts, tables, and explorers next to the claims they support, and keeps notebook IDs and other audit provenance in the storyboard, receipt, or an optional Sources & reproducibility disclosure. For predictive or scenario work, publication requires an evidence-bound claim contract that maps each material claim to its primary reader-facing visual or data surface, scope, caveat, uncertainty, and registered provenance. A causal claim also requires a declared causal design.

The renderer applies deterministic desktop composition frames: evidence surfaces use the report width while conclusions, findings, and methodology retain a readable measure. It checks this composition, spacing, insight density, and interpretation-rail balance in the desktop/webview smoke test. Where a selector and aggregate visual share a categorical field, the report can link the selector to the matching chart, table, and local interpretation; ambiguous relationships require an explicit selection contract. Chat and report cards distinguish a real publication from a draft, blocked gate, or failure—supplying a report path alone never marks a report as published.
