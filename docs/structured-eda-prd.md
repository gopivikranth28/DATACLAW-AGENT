...
| | |
|---|---|
| **Status** | In build (rev 8 — status refreshed 2026-07-13; P0–P6 and shared acceptance fixtures shipped; P7 UI and P8 eval harness remain) |
| **Owner** | Nandini Mathan |
| **Last updated** | 2026-07-13 |
| **Branch** | `structured-eda` (base: `release3` + cherry-picked structured-EDA skill from `eda-upgrade`) |
| **Ships as** | `plugins/dataclaw-eda/`, `plugins/dataclaw-analysis-review/`, patches to `plugins/dataclaw-plans/` and `plugins/dataclaw-notebooks/`, and `skill-library/{structured_eda,insight_validation,analysis_review,dataclaw}.md`; dedicated `ui/src` Findings surfaces and the `evals/` harness are remaining phases |
...
| Validation | deterministic tool/behavior suites and the live OpenClaw alias/manifest fixture are shipped; the golden end-to-end `evals/` harness (hypothesis-disposition scoring + opt-in LLM judge) remains P8 |

## Implementation baseline and current state

Verified on `structured-eda` (release3 base, refreshed 2026-07-13):

- **Plans gates shipped:** steps carry stable `plan_step_id` (`step-<hex8>`; legacy `id` coerced and stripped); `active_plan_context_hook` injects `session_id`/`proposal_id` and publishes `state["active_plan_step_id"]`; plan completion now evaluates the `ready_for_validation`/review gate and records explicit risk acceptance in an audit trail.
- **Artifacts and report spine shipped:** validator/wrapper hardening, versioned store with append-only living-report events, typed sections including `findings` with `plan_step_id` and preview caps (`TABLE_PREVIEW_MAX_ROWS = 20`, `TABLE_PREVIEW_MAX_BYTES = 50 KiB`); the workspace report path now designs, gates, and publishes structured HTML reports.
- **Skills and fixtures shipped:** hypothesis-driven `skill-library/structured_eda.md`, `insight_validation`, `analysis_review`, and DataClaw routing/mirrors; `examples/structured_eda/` retains the seeded customer-events fixture and expected behavior; deterministic tool, skill, and shared-contract tests cover the implemented path.
- **Sub-agent infra and reviewer are shipped:** `dataclaw/providers/sub_agent/{provider,registry}.py` and `DefaultSubAgentProvider` execute the registered `analysis-reviewer` definition; it uses a read-only metadata toolset, a six-turn bound, a 50 KiB-capped context manifest, and a labeled checklist-only degradation path when a provider is unavailable.
- **Remaining gaps:** a dedicated Findings/Review UI (P7) and an end-to-end/live behavioral evaluation harness (P8). The existing deterministic suites and OpenClaw schema fixture are not substitutes for P8's live-model evidence.

**Implementation update (2026-07-13):** P0–P6 are shipped. The branch contains the append-only hypothesis/finding ledgers and all eight EDA tools; notebook cell-id/source-hash evidence; loop observability (`loop_index`); selection and multiplicity floors; purpose-aware readiness; plan-gate enforcement and audited risk acceptance; the full deterministic review catalog and unresolved-review publish surfacing; and the explicit, bounded `analysis-reviewer` sub-agent. The review plugin preserves checklist findings on parse/provider failures and labels the resulting degradation, so a sub-agent-required scope cannot silently pass. The three shared FR-38 fixtures are also live: OpenClaw canonical/alias schema identity, stable `plan_step_id`, and the 20-row/50-KiB preview cap. The remaining delivery work is P7's dedicated Findings/Review UI and P8's committed golden/live evaluation harness.
...
- **Validation was unenforced pre-PRD.** Nothing distinguished a claim the agent recomputed and sanity-checked from one it eyeballed. This branch now blocks `ready_for_validation` through plan gates, the full deterministic checklist, and an explicit bounded reviewer sub-agent. Remaining work is the dedicated UI and end-to-end/live evaluation evidence.
...
Status column updated against the current structured-EDA branch state on 2026-07-13.
...
| P1 — EDA ledgers | plugin skeleton, both stores + enum constants, 8 tools + floors, AG-UI events, router, context hook | tool-level goldens green; caps + floors enforced; alias fixture passes | **Shipped** (`plugins/dataclaw-eda/`: store/tools/router/hooks + `tests/test_eda_tools.py`; alias fixture in `tests/prd_fixtures.py` + `tests/test_prd_shared_fixtures.py`) |
...
| P3 — Readiness | purpose/mode policies, hypothesis rollup, deferred-vs-unresolved distinction, artifact sections + living-report events | leakage blocks modeling; budget-deferred hypothesis is caveat not blocker; verdict persists supersedable | **Shipped** (`dataclaw_eda/readiness.py`, finding/readiness artifact-section wiring, and living-report events) |
...
| P6 — Reviewer sub-agent | definition + rubric rendering, direct-provider run, JSON parsing, degradation labeling | seeded fixture: unsupported claim + ledger-hygiene finding caught; checklist-only never passes required gate | **Shipped** (`dataclaw_analysis_review/reviewer.py`; `require_subagent` path in `request_analysis_review`; deterministic tests with a stubbed provider — the live-model catch rate is P8's job) |
...
5. **MVP cut if capacity forces it** — P0–P6 now ship the governed methodology: ledgers, readiness, full deterministic review, and bounded reviewer delegation. P7 UI and P8 live-eval scoring remain cut-compatible follow-on work.
