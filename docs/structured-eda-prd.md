...
| | |
|---|---|
| **Status** | In build (rev 7 — status refreshed 2026-07-10; P5 and P6 complete: full checklist + FR-30a publish surfacing + reviewer sub-agent) |
...
**Status update (2026-07-10):** P5 is complete. The checklist expansion shipped — `CHK-stale-evidence` (artifact-cited EDA findings with stale evidence anchors), `CHK-mlflow-repro` (model-scope runs missing params/metrics/non-system tags, read via a new sync failure-tolerant `session_run_metadata` helper in `dataclaw_plans/mlflow_tools.py`), and `CHK-open-required-on-ready` (a step marked `ready_for_validation` with open required review findings; excludes its own prior findings so it converges) — plus the FR-30a publish surfacing: a `surface_unreviewed_publish_hook` post-hook appends an "unresolved review risk" living-report event (log page, naming finding ids) when `publish_artifact` runs with open required findings, unless the step's gate risk carries an explicit acceptance. **P6 shipped the same day**: `dataclaw_analysis_review/reviewer.py` runs the `analysis-reviewer` definition (agent_type `llm`, `max_turns: 6`, read-only metadata toolset — no `read_artifact`, no data-query tools) through the sub-agent provider registry directly (D12); the rubric system prompt is rendered from the `analysis_review` skill at request time (installed body first, bundled fallback); the reviewer receives a structured, 50 KiB-capped context manifest (FR-26); fenced-JSON findings persist as `subagent:analysis-reviewer` records (signature-deduped, never auto-resolved on rerun); parse failures keep checklist findings and label the run `mixed` + `subagent_parse_error` (gate stays `unknown` for sub-agent-required scopes); a missing provider degrades to labeled checklist-only. **The three FR-38 shared acceptance fixtures also landed** (`tests/prd_fixtures.py`, exercised by `tests/test_prd_shared_fixtures.py`): `assert_openclaw_tool_aliases` runs the live `write_tool_manifest` and asserts schema identity for all 13 canonical tools, `assert_plan_step_identity` enforces `step-<hex8>` attribution on persisted evidence, and `assert_preview_cap` enforces the shared 20-row/50-KiB caps — all reusable by the P8 harness. Still open: P7 (UI) and P8 (evals).
