...
def _report_shell(*, title: str, first_section: str, include_plotly: bool = False) -> str:
    safe_title = html_lib.escape(title)
    plotly_script = _plotly_script_tag() if include_plotly else ""
    shell_css = _report_shell_css()
    shell_script = _report_shell_script()
...
def _ensure_plotly_runtime(doc: str) -> str:
    if 'data-dc-runtime="plotly"' in doc:
        return doc
    runtime = _plotly_script_tag()
    if "</head>" in doc:
