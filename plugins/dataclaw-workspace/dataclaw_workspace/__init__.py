        ...
        ctx.tool_registry.register_tool(PythonTool(
            ...
            parameters={
                ...
                "properties": {
                    ...
                    "visual_author": {"type": "object", "description": "Optional creative-author tuning. Every report is authored by the ledger-backed creative author from a persisted dossier of completed findings, caveats, ledger entries, and bounded aggregate values; source coverage, evidence review, one repair pass, CSP, and artifact safety are enforced, and a non-empty evidence ledger is required. This object only tunes timeout_seconds (default 600, max 900 — a per-call ceiling, raise for very large reports), max_output_chars (default/max 600000), max_dossier_chars (default 300000, max 600000), max_repair_passes (default 2, max 3), and max_repair_prompt_chars (default 700000; lower it to match a smaller provider context window). There is no deterministic or bounded mode."},
                },
